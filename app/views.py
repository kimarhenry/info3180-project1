...
def profile():
    ...
    if request.method == 'POST':
        userid=620000000+randint(10000,99999)
        ...
        user =UserProfile(id=userid,profile_creation=creation,first_name=fname,last_name=lname,bio=bio,imagename=profile_image,age=age,gender=gender)
        db.session.add(user)
        db.session.commit()
...
def profiles():
    ...
    for entry in entries: 
        username,usid =entry[0],entry[1]
        users+=[{"username":username,"userid":usid}]
        results ={"users":users}
...
def profile_userid(userid):
    ...
    for user in profile_info: 
        ...
        results={"userid":usid,"username":username, "image":profile_pic,"gender": gender, "age": age, "profile_created_on":created_on } 
    response =app.response_class(response=json.dumps(results),status=200, mimetype='application/json' )
    return response
